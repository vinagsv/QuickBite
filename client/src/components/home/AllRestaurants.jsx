...
const AllRestaurants = () => {
  ...
  const getRestaurants = () => {
    ...
    return restaurants.map((restaurant) => {
      const dishesForRestaurant = allDishes.filter(
        (dish) => dish.restaurantId === restaurant.id
      );
      const menuCount = dishesForRestaurant.length;
      const maxRating = dishesForRestaurant.reduce(
        (max, dish) => Math.max(max, dish.restaurantRating || 4.0),
        4.0
      );
